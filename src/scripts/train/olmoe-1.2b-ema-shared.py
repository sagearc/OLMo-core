"""
OLMoE 1.2B with EMA z-score normalized routing (port of the old olmo-rocm
``moe_router_ema_normalize`` path).

Same shape as ``olmoe-1.2b-baseline-shared.py`` (d_model=2048, n_layers=8, 8 experts,
top-2, dropless, shared expert), but the router z-normalizes its logits per expert
using an EMA of mean / E[X^2] before applying the softmax. The standard auxiliary
load-balancing loss is disabled; only the router z-loss is kept for logit-magnitude
stability (matches the old ``_router_ema_hook`` setup).

Usage:
    python src/scripts/train/olmoe-1.2b-ema-shared.py RUN_NAME [OVERRIDES...]
"""
...
def build_config(run_name: str, overrides: List[str]) -> ExperimentConfig:
    ...
    # Standard load-balance loss is replaced by per-expert z-normalization.
